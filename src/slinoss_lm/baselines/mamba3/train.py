...
from ...checkpoint import find_latest_checkpoint, load_checkpoint, restore_rng_state
from ...common import (
    append_jsonl,
    atomic_write_json,
    atomic_write_text,
...
from .config import Mamba3ExperimentConfig, config_to_dict, load_config
from .model import Mamba3CausalLM
from ...data import CudaPrefetcher, build_train_loader, load_packed_meta
...
def main() -> None:
    import slinoss_lm.train as train_runtime
    ...
    try:
        ...
        base_model = Mamba3CausalLM(config.model)
        if device.type == "cuda":
            torch.nn.Module.cuda(base_model, device.index or 0)
        else:
            torch.nn.Module.cpu(base_model)
        raw_model = maybe_compile(base_model, cast(Any, config))
        ...
        scheduler = CosineSchedule(cast(Any, config), global_batch_tokens)
        ...
        if is_main_process():
            wandb_logger = build_wandb_logger(
                ...
                run_metadata={
                    ...
                    "baseline_family": "mamba3",
                    "baseline_variant": "siso" if not config.model.is_mimo else "mimo",
                    "world_size": world_size,
        ...
        while tokens_consumed < config.train.target_tokens:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                base_model.parameters(), config.optim.grad_clip_norm
            )
            ...
            if step % config.logging.log_every_steps == 0:
                ...
                if is_main_process():
                    ...
                    if device.type == "cuda":
                        logger.info(
                            "step=%s loss=%.4f lr=%.3e grad_norm=%.3f tokens=%s toks/s=%.1f step=%.3fs peak_mem=%.2fGiB",
                            format_int(step),
                            avg_loss,
                            lr,
                            float(grad_norm),
                        ...
                        logger.info(
                            "step=%s loss=%.4f lr=%.3e grad_norm=%.3f tokens=%s toks/s=%.1f step=%.3fs",
                            format_int(step),
                            avg_loss,
                            lr,
                            float(grad_norm),
            ...
            if should_validate and is_main_process():
                metrics = run_validation(
                    base_model, config=cast(Any, config), device=device
                )
            ...
            if train_runtime.STOP_REQUESTED and config.checkpoint.save_on_signal:
                should_save = True
            if world_size > 1:
        ...
        if is_main_process():
            ...
            if wandb_logger is not None:
                summary_payload: dict[str, int | float | bool | str] = {
                    "final_step": step,
